...
logging.basicConfig(level=logging.INFO,
                    filename='logs/splitCashflow.log',
...
def splitSingleFile(filename, outputpath):
    ...
    for code in codes:
        if os.path.exists(os.path.join(outputpath, code+'.csv')):
            ...
            files[code].write('ANN_DT,REPORT_PERIOD,cashflow\n')
        data[code] = []
    for index, row in df.iterrows():
        code = row['WIND_CODE']
        #files[code].write(str(row['Date'])+','+str(row['s_val_mv'])+'\n')
        data[code].append((row['ANN_DT'], row['REPORT_PERIOD'], row['NET_CASH_FLOWS_OPER_ACT']))
    for code in codes:
