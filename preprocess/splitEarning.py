...
logging.basicConfig(level=logging.INFO,
                    filename='logs/splitEarning.log',
...
def splitSingleFile(filename, outputpath):
    ...
    for code in codes:
        if os.path.exists(os.path.join(outputpath, code+'.csv')):
            ...
            files[code].write('ANN_DT,REPORT_PERIOD,earning\n')
        data[code] = []
    for index, row in df.iterrows():
        code = row['S_INFO_WINDCODE']
        #files[code].write(str(row['Date'])+','+str(row['s_val_mv'])+'\n')
        data[code].append((row['ANN_DT'], row['REPORT_PERIOD'], row['NET_PROFIT_EXCL_MIN_INT_INC']))
    for code in codes:
